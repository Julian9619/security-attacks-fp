...
namespace gazebo
{
    class Postbox1Model : public ModelPlugin
    {
        public:Postbox1Model() : ModelPlugin(){
        ...
        */
        ...
        void Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
        {
            ...
            std::bind(&Postbox1Model::OnUpdate, this));
            ...
            ros::SubscribeOptions so = ros::SubscribeOptions::create<std_msgs::Int8MultiArray>(
                "/to_postbox1", 1,
                boost::bind(&Postbox1Model::OnRosMsg, this, _1), 
                ros::VoidPtr(),&rosQueue);

            rosSub = rosNode->subscribe(so);
            rosQueueThread =std::thread(std::bind(&Postbox1Model::QueueThread, this));

            sender_pub = n.advertise<std_msgs::Int8MultiArray>("from_postbox1", 1000);
        ...
        {
            //ROS_WARN("postbox1 received: %d %d", msg->data[0], msg->data[1]);
            int num = 0;
            if(msg->data[0]==1){
                if(msg->data[1]==1){
    ...
    GZ_REGISTER_MODEL_PLUGIN(Postbox1Model)
}